...
export function Storefront({ products, slides: initialSlides, settings }: Props) {
  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <section id="inicio" className="bg-primary px-4 pb-5 pt-3 text-primary-foreground md:px-10 md:py-10"><div className="mx-auto max-w-7xl overflow-hidden rounded-[2rem] bg-[#1197c5] shadow-lg"><div className="relative h-[300px] w-full overflow-hidden sm:h-[420px] lg:h-[520px]">
      ...
      <div className="absolute bottom-4 left-1/2 z-20 flex -translate-x-1/2 gap-2">{slides.map((_, index) => <motion.button key={index} aria-label={`Ir al slide ${index + 1}`} onClick={() => setSlide(index)} animate={slide === index ? { scale: 1.4, backgroundColor: '#ffffff' } : { scale: 1, backgroundColor: 'rgba(255,255,255,0.6)' }} transition={{ type: 'spring', stiffness: 400, damping: 20 }} className="size-2.5 rounded-full" />)}</div>
    </div></div></section>
    <section id="catalogo" className="relative mx-auto max-w-7xl px-5 py-12 md:px-10 md:py-20">
      <motion.div className="mb-10 flex flex-col gap-6 md:flex-row md:items-end md:justify-between" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.5 }}>
        <div>
          <motion.p className="mb-3 inline-flex items-center gap-2 rounded-full bg-[#1197c5]/10 px-3.5 py-1.5 text-xs font-semibold uppercase tracking-[0.18em] text-[#1197c5]"><Droplets className="size-3.5" /> Colección mineral</motion.p>
          <h2 className="font-serif text-4xl tracking-tight md:text-5xl">Elige tu <span className="italic text-[#1197c5]">origen</span></h2>
        ...
        <div className="flex items-center gap-4">
          <label className="flex min-w-0 flex-1 items-center gap-3 rounded-full border border-border bg-background px-4 py-2.5 text-sm text-muted-foreground shadow-sm transition-colors focus-within:border-[#1197c5] focus-within:ring-2 focus-within:ring-[#1197c5]/20 md:w-64"><Search className="size-4 shrink-0" /><span className="sr-only">Buscar productos</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar agua" className="w-full bg-transparent outline-none placeholder:text-muted-foreground" /></label>
          <div className="flex gap-1 rounded-full border border-border bg-background p-1 shadow-sm" aria-label="Cambiar distribución">{['grid', 'list'].map((mode) => <motion.button key={mode} aria-label={mode === 'grid' ? 'Vista en cuadrícula' : 'Vista en lista'} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.85 }} className={`rounded-full p-2 transition-colors ${view === mode ? 'bg-[#1197c5] text-white shadow' : 'text-muted-foreground'}`} onClick={() => setView(mode as 'grid' | 'list')}>{mode === 'grid' ? <Grid2X2 className="size-4" /> : <List className="size-4" />}</motion.button>)}</div>
        </div>
      </motion.div>
      <motion.div variants={staggerContainer} initial="hidden" animate="show" className={view === 'grid' ? 'grid grid-cols-2 gap-x-4 gap-y-10 sm:grid-cols-3 lg:grid-cols-4 lg:gap-x-6 lg:gap-y-14' : 'flex flex-col gap-3'}>
        {filtered.map((product) => <motion.article key={product.id} layout variants={cardVariants} whileHover={view === 'grid' ? { y: -12 } : { x: 8 }} whileTap={{ scale: 0.98 }} transition={spring} className={view === 'grid' ? 'group cursor-pointer' : 'flex cursor-pointer items-center gap-4 rounded-2xl border border-border bg-background p-4 shadow-sm transition-shadow hover:shadow-md'} onClick={() => setSelected(product)}>
          <div className={view === 'grid' ? 'relative mb-4 aspect-[4/5] overflow-hidden rounded-[1.6rem] bg-gradient-to-b from-secondary to-background shadow-sm transition-shadow duration-300 group-hover:shadow-xl group-hover:shadow-primary/10' : 'relative size-20 shrink-0 overflow-hidden rounded-2xl bg-gradient-to-b from-secondary to-background sm:size-24'}>
            <motion.img src={product.featuredImage?.url || '/placeholder.jpg'} alt={product.featuredImage?.altText || product.title} className="size-full object-cover mix-blend-multiply" whileHover={{ scale: 1.12 }} transition={{ duration: 0.55, ease: 'easeOut' }} />
            <span className="absolute left-3 top-3 rounded-full bg-background/85 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-primary backdrop-blur">{product.tags[0] || 'Mineral'}</span>
            <motion.span whileHover={{ scale: 1.15 }} whileTap={{ scale: 0.9 }} className="absolute bottom-3 right-3 grid size-10 place-items-center rounded-full bg-[#1197c5] text-white opacity-0 shadow-lg transition-opacity duration-300 group-hover:opacity-100"><Plus className="size-4" /></motion.span>
          </div>
          <div className={view === 'grid' ? 'min-w-0 px-1' : 'min-w-0 flex-1'}>
            <div className="flex items-start justify-between gap-2"><h3 className={`font-serif leading-tight transition-colors group-hover:text-[#1197c5] ${view === 'grid' ? 'text-xl' : 'text-2xl'}`}>{product.title}</h3><p className="shrink-0 font-bold text-[#1197c5]">{formatPrice(product.priceRange.minVariantPrice.amount, product.priceRange.minVariantPrice.currencyCode)}</p></div>
